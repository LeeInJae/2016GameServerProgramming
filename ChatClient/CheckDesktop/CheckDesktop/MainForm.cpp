...
void MainForm::PacketProcess()
{
	...
	{
		m_pClientSceen->ProcessPacket(packet.PacketID, packet.p_Data);
		m_pClientSceenLogin->ProcessPacket(packet.PacketID, packet.p_Data);
		m_pClientSceenLobby->ProcessPacket(packet.PacketID, packet.p_Data);

		if (packet.p_Data != nullptr) {
			delete[] packet.p_Data;
		}
	}
